...
export default {
    ...
    actions: {
        ...
        async guessMatch({ dispatch, commit, state }, payload) {
            ...
            const { data, code } = await guessMatch(payload).catch(err => {
                Toast('网络开小差啦，请稍后再试')
                return;
            });
            if (code == 0) {
